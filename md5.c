...
void cacl_md5(char* enc_buf, char* dec_buf)
{
	...
	if(NULL == enc_buf)
	{
		DEBUG_OUT("encrypt buffer error\n");
		return;
	}

	if(NULL == dec_buf)
	{
		DEBUG_OUT("decrypt buffer error\n");
		return;
	}

	MD5_CTX md5;
	MD5Init(&md5);
	MD5Update(&md5, enc_buf, strlen(enc_buf));
	MD5Final(&md5, decrypt);

	int i = 0;
	for (i=0; i<16; i++)
	{
        sprintf(c, "%02x",decrypt[i]);
        strcat(dec_buf, c);
	}
...
{
    int i;
    //unsigned char encrypt[] ="admin";//21232f297a57a5a743894a0e4a801fc
